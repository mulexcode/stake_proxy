...
async function cash_out() {
    ...
    const [chainConfig, ] = anchor.web3.PublicKey.findProgramAddressSync([Buffer.from("chain_config"), new BN(103).toArrayLike(Buffer, 'le', 8)], program.programId);
    ...
    const tx = await program.methods.cashOut({
        tokenName: "mix",
        target: payer.publicKey.toString(),
        amount: new BN(10000000),
        targetChainId: new BN(103),
    }).accountsPartial({
